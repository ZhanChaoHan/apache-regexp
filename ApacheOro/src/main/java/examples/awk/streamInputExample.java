...
/**
 * This is a test program demonstrating how to search an input stream with the
 * AwkTools regular expression classes.
 *
 * @version @version@
 */
public final class streamInputExample {
	...
	/**
	 * This program extracts sentences containing the word C++ from the sample file
	 * streamInputExample.txt The regular expression used is not perfect, so focus
	 * on AwkStreamInput and not the ability of the regular expression to handle all
	 * normal sentences. For those not familiar with the OROMatcher Util class, a
	 * use of the Util.substitute method is included.
	 */
	public static final void main(String args[]) {
		...
		try {
			...
			while (matcher.contains(input, pattern)) {
				...
				sentence = Util.substitute(matcher, newline, new StringSubstitution(" "), result.toString(),
						Util.SUBSTITUTE_ALL);
				System.out.println("\nMatch:\n" + sentence);
			}
		} catch (IOException e) {
